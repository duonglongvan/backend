...
class JSearchService {
    async get(id,user) {
        const sql="SELECT a.* FROM parsing_rule AS a WHERE a.id="+id;
        const data = await global.mariadb.query(sql);
    ...
    async gets(req) {
        ...
        const sql="SELECT a.* FROM parsing_rule AS a WHERE a.id>0 LIMIT "+offset+","+limit;
        const data = await global.mariadb.query(sql);
