...
let kafka_host;
 class JKafka {
  async connection() {
    kafka_host = process.env.KAFKA_HOST + ':' + process.env.KAFKA_PORT;
    try {
  ...
  //  logger.info("optionKafka iss ",optionKafka);
    kafkaClient = new Kafka(optionKafka);
    logger.info("Kafka  on ready");
    ...
    logger.err("Kafka  not ready ",err.message);
  }
  }
